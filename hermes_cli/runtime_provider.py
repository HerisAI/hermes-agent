...
def _copilot_runtime_api_mode(
    ...
    """Resolve the Copilot API surface for the model that will actually run.

    ``effective_model`` is the explicitly requested model (``-m``/``--model``,
    a mid-session ``/model`` switch, or a per-task pin) and takes precedence
    over the persisted ``model.default``.  Without it the mode is derived from
    whatever the config happens to hold, which silently routes a
    Responses-only model (``gpt-5.5``, the ``gpt-5.6-*`` family, ``grok-4.5``)
    to ``/chat/completions``.  That endpoint answers 400
    ``unsupported_api_for_model``, which surfaces to the user as an empty
    completion with exit code 0 — no output and no error.
    """
...
def _resolve_explicit_runtime(
    *,
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if provider == "copilot":
            api_mode = _copilot_runtime_api_mode(
                model_cfg, api_key, effective_model=target_model
            )
        elif provider == "xai":
            api_mode = "codex_responses"
        else:
